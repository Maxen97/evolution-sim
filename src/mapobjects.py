...
class Creature(MapObject):
	def __init__(self, drawer):
		...
		self.energy = 100 #creature dies when reaching energy reaches 0
	...
	def update(self, foods, creatures):
		#CALCULATING COLLISION BETWEEN SIGHT AND FOODS
		objects_in_sight = []
		for food in foods: #THIS LOOP CAN BE OPTIMIZED BY CALCULATING x2 AND y2 ONCE
			x0 = food.x
			y0 = food.y
			x1 = self.x
			y1 = self.y
			x2 = self.x + self.sight * math.cos(self.direction)
			y2 = self.y + self.sight * math.sin(self.direction)
			d = util.distanceLinePoint(x1,y1,x2,y2,x0,y0)
